...
返回值比较特殊：exec 函数一旦调用成功**，即执行新的程序，不返回**。只有失败才返回，错误值-1，所以通常我们直接在 exec 函数调用后直接调用 perror()，和 exit()，无需 if 判断。
...
第三个参数可以通过WNOHANG来指定非阻塞回收，而wait函数只能是**阻塞回收**。
